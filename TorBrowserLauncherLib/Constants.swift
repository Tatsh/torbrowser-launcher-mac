...
let kBackgroundIdentifier = "\(Bundle.main.bundleIdentifier!).background"
/// File URI to hdiutil.
...
let kUpdateIndexURI = URL(string: "\(kUpdateURIPrefix)?C=M;O=D")!
...
let kTorBrowserAppURI = URL(fileURLWithPath: kTorBrowserAppPath, isDirectory: true)
...
let kTorBrowserVersionURI = URL(fileURLWithPath: kTorBrowserVersionPath)
...
/// File URI to xattr.
let kXAttrURI = URL(fileURLWithPath: "/usr/bin/xattr")
