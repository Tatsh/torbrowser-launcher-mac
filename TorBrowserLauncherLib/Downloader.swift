...
private func findUpdatePath(_ data: Data) throws -> String {
    guard let html = String(data: data, encoding: .utf8),
        let updatePath = try findMatchInLines(lines: html, regex: kUpdateRE)
    else {
        throw NSError(
            domain: "TBLDownloader", code: DownloaderFailedToFindUpdatePathError,
            userInfo: [NSLocalizedDescriptionKey: "Failed to find update path in HTML."])
...
open class Downloader {
    ...
    public func download() throws {
        ...
        Task {
            ...
            let (data, resp) = try await getContent(session: session, url: updateIndexURI)
            ...
            fileManager.createDirectoryIgnoreError(
            ...
            let binary = try await getBinaryURI(session: session, updatePath)
